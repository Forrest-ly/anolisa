...
| WorkBuddy | `workbuddy` | Hard-disabled | Passes Bash input through by default; `TOKENLESS_WORKBUDDY_AUTO_ALLOW=1` opts in to replacing it via `modifiedInput` | Replaces output on CodeBuddy Code CLI hosts; other WorkBuddy hosts pass through unchanged | Attempted after response compression | — |
...
User-configured hooks and every other settings key are preserved; the uninstall script removes only the Tokenless-owned entries. Both scripts rewrite `settings.json` through a temporary file and never loosen the existing file mode, because the `.codebuddy` home may carry credentials (`settings.json.env` officially supports `CODEBUDDY_API_KEY` and auth tokens); a newly created `settings.json` defaults to `0600`. The staged file must also come back with the original owner and group: when that restore is not permitted — a cross-account run without privilege to `chown` — both scripts abort with `cannot preserve the ownership of …` and leave the existing `settings.json` untouched instead of replacing it with a file whose `0600` mode the real owner may no longer be able to read. Re-run the script as the file's owner.
