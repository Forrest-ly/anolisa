...
The shared response hook, OpenClaw, and Hermes skip inputs shorter than 200 characters. Codex skips inputs shorter than 500 characters; it includes compressed content only for inputs of at least 4,000 characters and otherwise adds diagnostics or a summary. Skill-like text with YAML frontmatter is also skipped by the shared paths. The TOON encoding step only runs on payloads of at least 500 characters (the current implementation threshold, which may be adjusted later); smaller payloads keep the compressed form, because TOON savings on small JSON are negligible. The threshold applies to every TOON-capable pipeline: the shared response hook, the standalone TOON hook, Codex, OpenClaw, and Hermes. The runtime enforces the same threshold, so calling `tokenless compress-toon` (or the SDK `compress_toon`) directly on a payload shorter than 500 characters passes it through unchanged instead of encoding it. A payload skipped by the threshold is expected behavior, not a failure: it records no TOON statistics row, so a missing stats row for a short payload does not mean compression failed.
