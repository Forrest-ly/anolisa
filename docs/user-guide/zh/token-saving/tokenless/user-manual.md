...
- **可逆压缩（Stash）**：响应/Schema 压缩默认 *inline 有损、端到端无损*——被截断的内容写入 stash（BLAKE3 键），输出嵌入 `<<tokenless:KEY>>` 标记，LLM 可按需 `tokenless retrieve` 取回原文。
...
| 可逆压缩（Stash） | 端到端无损 | 截断内容写入 stash，嵌入 `<<tokenless:KEY>>` 标记，LLM 可按需 retrieve 原文 |
...
| `--no-stash` | 关闭可逆 stash：截断变为有损不可检索（pre-stash 行为） |
| `--stash-db <path>` | 覆盖 stash 数据库路径（默认 `~/.tokenless/stash.db`，须位于用户真实 home 下） |
...
响应/Schema 压缩默认将被截断的内容写入 stash，并在输出嵌入 `<<tokenless:KEY>>` 标记。LLM 引用该标记即可取回原文：
...
`hash` 为 24 个十六进制字符（BLAKE3 前 12 字节 / 96 位）。stash 条目带 TTL（SQLite 后端 1 小时，InMemory 后端 5 分钟），过期后 retrieve 返回空。详见 [工作原理 · 可逆压缩](#可逆压缩stash)。

### MCP 服务

```bash
# 启动 stdio MCP 服务，暴露 tokenless_retrieve 工具
tokenless mcp serve
```

MCP 连接的 Agent 可通过 `tokenless_retrieve` 工具按需恢复被截断的 payload，无需 shell 调用 CLI。
...
|------|------|---------|---------|---------|
| R1 | 字符串截断 | 长度 > 4096 字符 | UTF-8 安全边界截断；默认 stash 原文并追加 `… (truncated, retrieve with <<tokenless:KEY>>)`，`--no-stash` 时退化为 `… (truncated)` | 4096 字符 |
| R2 | 数组截断 | 元素 > 32 | 保留前 32 个；默认 stash 截断尾部并追加 `<... N items truncated, retrieve with <<tokenless:KEY>>`，`--no-stash` 时退化为 `<... N more items truncated>` | 32 个 |
| R3 | 字段删除 | key 匹配黑名单 | 整个字段移除 | 7 个字段 |
| R4 | null 移除 | 值为 `null` | 从对象/数组删除 | 启用 |
| R5 | 空值移除 | 值为 `""`/`[]`/`{}` | 从对象/数组删除 | 启用 |
| R6 | 深度截断 | 嵌套深度 > 8 | 默认 stash 子树并替换为 `<{type} truncated at depth {N}, retrieve with <<tokenless:KEY>>`，`--no-stash` 时退化为 `<{type} truncated at depth {N}>` | 8 层 |
...
响应/Schema 压缩是 *inline 有损、端到端无损*：截断时被丢弃的 payload 序列化为 JSON，以 BLAKE3 派生的 24-hex 键存入 stash，输出嵌入 `<<tokenless:KEY>>` 标记。LLM 引用标记即可取回原文，因此 inline 表示更小但信息不永久丢失。机制对标 Headroom 的 CCR（Compress-Cache-Retrieve），此处命名 **stash** 以避用专有缩写。

- **键空间**：键为 payload 的 BLAKE3 前 12 字节（96 位），生日界 2⁴⁸，视作唯一句柄。`tokenless:` 命名空间区分于用户内容。
- **后端**：`SqliteStore`（feature `sqlite`，默认开启，持久化到 `~/.tokenless/stash.db`，WAL）为生产推荐——hook 每次 fork+exec 新进程，内存后端跨进程丢失；`InMemoryStore` 用于测试/单进程。
- **TTL**：条目带过期时间，过期自动清理（SQLite 1 小时，InMemory 5 分钟）；过期后 retrieve 返回空。
- **关闭**：`--no-stash` 使截断有损不可检索（pre-stash 行为）；未附加 stash store 时同样退化为有损。
- **取回**：`tokenless retrieve <KEY>`，或经 MCP `tokenless_retrieve` 工具（`tokenless mcp serve`）。`retrieve` 接受裸哈希或含 `<<tokenless:HASH>>` 标记的整行文本（自动提取）。

详见 `docs/stash-reversible-compression.md`。源码：`crates/tokenless-ccr/`。
...
- **Stash 取回**：条目过期（SQLite 1h / InMemory 5min）或 `--no-stash` 截断后 retrieve 返回空；stash 不可用时压缩退化为有损，不阻塞输出
...
| `retrieve` 返回空 | 条目已过 TTL（SQLite 1h / InMemory 5min）；或压缩时用了 `--no-stash`；或 `--stash-db` 指向了不同的库 |
...
```bash
sudo anolisa --install-mode system repair tokenless  # 协调 system 状态
sudo anolisa --install-mode system forget tokenless  # 只删除 system 记录
sudo anolisa --install-mode system adopt tokenless   # 重新纳管仍存在的 RPM
```
...
| PostToolUse | `compress_response_hook.py` | 响应压缩（默认可逆 stash）+ TOON + 失败归因 | — | 10000ms |
...
| stash 数据库（默认） | `~/.tokenless/stash.db` |
| 可逆压缩（stash）实现 | `crates/tokenless-ccr/` |
...
**版本**：0.7.0
**文档版本**：2.2（补充可逆压缩/Stash、retrieve 与 mcp 子命令，对齐 0.7.0 实现）
