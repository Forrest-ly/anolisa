...
输出：
```json
[1, 2, "<... 7 more items truncated>", 10]
```

采用首尾采样策略：保留数组头部和尾部元素，丢弃中间部分。截断标记（marker）插入在头部样本与尾部样本之间，保持原始顺序语义。被丢弃的中间段（本例中为 `[3, 4, 5, 6, 7, 8, 9]`）在启用 stash 时完整保存，可通过 marker 中的 hash 检索还原。这确保最近的内容（如末尾的错误条目、最新的 diff hunk）不会被无差别截断。默认阈值 32 个元素。
