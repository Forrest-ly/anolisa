...
# These helpers adapt hook output format for Cosh-NG compatibility.
...
def extract_llm_content(input_data: dict) -> str | None:
    ...
    Returns ``None`` if tool_response is not a Cosh-NG wrapper or
    ``llmContent`` is missing/empty.
    """
    tool_response = input_data.get("tool_response")
    if not isinstance(tool_response, dict):
        return None
...
def cosh_ng_supports_replacement() -> bool:
    ...
    Returns ``True`` only if Cosh-NG version is detected and >=
    ``COSH_NG_MIN_REPLACEMENT_VERSION``.

    Returns ``False`` if the version is too old, missing, or unparseable,
    so callers can fail open and avoid duplicate content injection.
    """
